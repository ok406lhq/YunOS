  ...
  $.fn.Page = function (options) {
    ...
    return this.each(function () {
      ...
      obj.on('click', '.next', function () {
        var pageshow = parseInt($(this).siblings('ul').find('.' + active).html());
        var nums,flag;
        var a = n % 2;
        if(a == 0){
          //偶数
        ...
        if(pageshow >= l) {
          ...
          //最前几项
          $(this).siblings('ul').find('.' + active).removeClass(active).parent().next().find('a').addClass(active);
        ...
        opts.callBack(pageshow+1,$(this).closest(".page"));
      });
      obj.on('click', '.prv', function () {
        ...
        if (pageshow <= 1) {
          ...
          //最前几项或最后几项
            $(this).siblings('ul').find('.' + active).removeClass(active).parent().prev().find('a').addClass(active);
        }else {
            $(this).siblings('ul').find('.' + active).removeClass(active).parent().prev().find('a').addClass(active);
          fpageShow(pageshow-1);
        }
      ...
      obj.on('click', '.first', function(){
        var activepage = parseInt($(this).siblings('ul').find('.' + active).html());
        if (activepage <= 1){
          return
        }//当前第一页
        opts.callBack(1,$(this).closest(".page"));
        fpagePrv(0);
      });
      obj.on('click', '.last', function(){
        ...
        opts.callBack(l,$(this).closest(".page"));
        if(l>n){
          fpageNext(n-1);
        }else{
          fpageNext(l-1);
      ...
      obj.on('click', 'li', function(){
        ...
        var nums = odevity(n);
        opts.callBack(pageshow,$(this).closest(".page"));
        if(l>n){
