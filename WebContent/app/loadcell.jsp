...
<body>
			...
			<form role="form" class="form-inline search_form">
				<div class="form-group">
					<label for="" class="temp_to">固定&nbsp;Y轴：</label> <input
						type="checkbox" class="static_checkbox"
	...
	$.post("./../LoadCellServlet",data,function(data){
		var data = eval(data);
			...
			$.post("./../LoadCellServlet",data,function(data){
				...
				var temp = 0;
					for (var i = data.length - 1; i >= 0; i--) {
						...
						temp += data[i].car_weight;
					}
					if ($("#isY").is(":checked")) {
						if(numY == null){
							numY = Math.ceil(((temp/data.length)*2)/10)*10;//取平均数
							$(".chartY").val(numY);
						}
						scale = "";
			        }
					...
					var time = [];
					$.each(data,function(index,result){
						car_weight.push(result.car_weight);
						axis_weight_1.push(result.axis_weight_1);
						axis_weight_2.push(result.axis_weight_2);
						axis_weight_3.push(result.axis_weight_3);
						axis_weight_4.push(result.axis_weight_4);
						axis_weight_5.push(result.axis_weight_5);
						axis_weight_6.push(result.axis_weight_6);
						axis_weight_7.push(result.axis_weight_7);
						axis_weight_8.push(result.axis_weight_8);
						axis_weight_9.push(result.axis_weight_9);
						axis_weight_10.push(result.axis_weight_10);
					...
					option = {
						...
						legend: {
							data:['车重','轴1重','轴2重','轴3重','轴4重','轴5重','轴6重','轴7重','轴8重','轴9重','轴10重'],
							orient:"verticality", 
							left: 'right',
							top: 49,
						    tooltip: {
