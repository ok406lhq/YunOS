...
public class LrdmResultDao {

	public LrdmResultDao() {
		super();
	}
	 ...
	 * @throws Exception
	 */
	public  String queryLrdmResultJson(Connection conn, String prjName, 
		...
		String url = Connest.URL_LRDM_QueryLrdmResultJson;
...
//		
//			sql="select DATE_FORMAT((sDateTime),'%Y-%m-%d %H') as date, min(displacement) as displacement,FORMAT(avg(temperature),1) as temperature  from " + ptInfo.getStaName() + "_" + ptInfo.getSensorName()+
//			 " where  sdateTime>\""+starttime+"\" and sdateTime<\""+endtime+"\" and mark < 4 group by date";
//			
//		}else if(ymw.equals("day")){
//			 sql="select DATE_FORMAT((sDateTime),'%Y-%m-%d') as date, min(displacement) as displacement,FORMAT(avg(temperature),1) as temperature  from " + ptInfo.getStaName() + "_" + ptInfo.getSensorName()+
//			 " where  sdateTime>\""+starttime+"\" and sdateTime<\""+endtime+"\" and mark < 4 group by date";
//				
//		}
//		else if(ymw.equals("month")){
//			sql="select DATE_FORMAT((sDateTime),'%Y-%m') as date, min(displacement) as displacement,FORMAT(avg(temperature),1) as temperature  from " + ptInfo.getStaName() + "_" + ptInfo.getSensorName()+
//			 " where  sdateTime>\""+starttime+"\" and sdateTime<\""+endtime+"\" and mark < 4 group by date";
//		}else if(ymw.equals("year")){
//			sql="select DATE_FORMAT((sDateTime),'%Y') as date, min(displacement) as displacement,FORMAT(avg(temperature),1) as temperature  from " + ptInfo.getStaName() + "_" + ptInfo.getSensorName()+
//			 " where  sdateTime>\""+starttime+"\" and sdateTime<\""+endtime+"\" and mark < 4 group by date";
//		}
//		
//		//System.out.print(sql);
