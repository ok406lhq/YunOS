...
public class PiezometerResultDao {

	public PiezometerResultDao() {
		super();
	}
	 ...
	 * @throws Exception
	 */
	public  String queryPiezometerResultJson(Connection conn, String prjName, 
		...
		String url = Connest.URL_PIEZOMETER_QueryPiezometerResultJson;
...
//		
//			sql="select DATE_FORMAT((sDateTime),'%Y-%m-%d %H') as date, max(wheight) as wheight,FORMAT(avg(temperature),1) as temperature  from " + ptInfo.getStaName() + "_" + ptInfo.getSensorName()+
//			 " where  sdateTime>\""+starttime+"\" and sdateTime<\""+endtime+"\" and mark < 4 group by date";
//			
//		}else if(ymw.equals("day")){
//			 sql="select DATE_FORMAT((sDateTime),'%Y-%m-%d') as date, max(wheight) as wheight,FORMAT(avg(temperature),1) as temperature  from " + ptInfo.getStaName() + "_" + ptInfo.getSensorName()+
//			 " where  sdateTime>\""+starttime+"\" and sdateTime<\""+endtime+"\" and mark < 4 group by date";
//				
//		}
//		else if(ymw.equals("month")){
//			sql="select DATE_FORMAT((sDateTime),'%Y-%m') as date, max(wheight) as wheight,FORMAT(avg(temperature),1) as temperature  from " + ptInfo.getStaName() + "_" + ptInfo.getSensorName()+
//			 " where  sdateTime>\""+starttime+"\" and sdateTime<\""+endtime+"\" and mark < 4 group by date";
//		}else if(ymw.equals("year")){
//			sql="select DATE_FORMAT((sDateTime),'%Y') as date, max(wheight) as wheight,FORMAT(avg(temperature),1) as temperature  from " + ptInfo.getStaName() + "_" + ptInfo.getSensorName()+
//			 " where  sdateTime>\""+starttime+"\" and sdateTime<\""+endtime+"\" and mark < 4 group by date";
//		}
//		
//		//System.out.print(sql);
