...
public class DismeterRecordService {


    public JSONArray getDismeterRecordList(String nSensorID, String startTime, String endTime) {
        if (StringUtils.isNotBlank(nSensorID)) {
            DismeterRecordDao dao = new DismeterRecordDao();
            ...
            List<Map<String,Object>> list = dao.getDismeterRecordList(nSensorID, startTime, endTime);
            if (list != null && !list.isEmpty()) {
                JSONArray array = new JSONArray();
                for (Map<String, Object> map : list) {
                    JSONObject object = new JSONObject();
                    object.put("createTime",DateUtil.dateTypeToString((Date) map.get("createTime"),"yyyy-MM-dd HH:mm:ss"));
                    object.put("displacement",map.get("displacement"));
                    array.add(object);
                }
                return array;
            }
